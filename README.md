...
* **Code Architecture:** Refactored from procedural code to reusable functions (DRY principle).
...
1.  **DOM Events:** The difference between `keydown` and `input` events for real-time data handling.
2.  **Refactoring:** How to turn repetitive code into a clean, reusable `renderDice()` function.
3.  **Scope:** Managing variables properly to avoid global scope pollution.
4.  **Git Workflow:** Using branches and commit messages effectively.
